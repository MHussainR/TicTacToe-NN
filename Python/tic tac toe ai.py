    ...
    for i in range(len(board)):
        for j in range(len(board[i])):
            if type(board[i][j]) != str:
                moves += [board[i][j] for k in range (10)]
    return moves

def main ():

    boards = []
    data = open(r"boards.txt", "r")
    for i in data:
    ...
    while run:
        ...
        if not board in boards:
            ...
            data = open (r'learn.txt', 'a')
            data.write(str([board, moves_list(board)]))
            data.write('\n')
            data.close()
            data = open (r'boards.txt', 'a')
...
for i in range(5):
    ...
    if a == 'draw':
        ...
        u += 1
    cnt = []
    data = open (r'count.txt', 'r')
    for i in data:
        cnt.append(int(i))
    data.close()
    data = open (r'count.txt', 'w')
